...
USE_SBER_SERVER = False  # ← Переключатель

if USE_SBER_SERVER:
    QDRANT_HOST = "sc-gwd-llm-dv01"
    QDRANT_PORT = 6334
    QDRANT_TIMEOUT = 300
    ...
    QDRANT_PORT = 6333  # Для HTTP
    QDRANT_TIMEOUT = 300
    QDRANT_PREFER_GRPC = False  # Для localhost обычно HTTP
    QDRANT_API_KEY = None
    QDRANT_HTTPS = False


REPORTS_DIR = Path("qdrant-report-builder/generated_reports")
...
def get_collection_schema(collection_name):
    ...
    try:
        ...
        # 🔍 Увеличим лимит выборки для лучшего покрытия полей
        sample_limit = 50  # вместо 10
        ...
        # 🔍 Улучшенное извлечение полей с обработкой вложенных структур
        payload_fields = set()
        field_types = {}
        field_samples = {}  # Примеры значений для отладки
        ...
        for point in points:
            ...
            for key, value in point.payload.items():
                ...
                # 🔍 Безопасное определение типа с обработкой сложных случаев
                if value is None:
                    field_types[key] = "null"
                elif isinstance(value, (list, tuple)):
                    # Для списков: определяем тип первого не-None элемента
                    sample = next((v for v in value if v is not None), None)
                    field_types[key] = (
                        f"list[{type(sample).__name__}]" if sample else "list[null]"
                    )
                ...
                # Сохраняем пример значения (для отладки)
                if key not in field_samples and value is not None:
                    field_samples[key] = str(value)[:100]  # Обрезаем длинные значения

        # 🔍 Извлечение информации о векторах
        points_count = getattr(info, "points_count", None)
        indexed_vectors_count = getattr(info, "indexed_vectors_count", None)

        vector_config = getattr(info, "config", None)
        vectors_info = {}
        if vector_config and hasattr(vector_config, "params"):
            vectors_param = getattr(vector_config.params, "vectors", None)
            if isinstance(vectors_param, dict):
                for name, config in vectors_param.items():
                    vectors_info[name] = {
                        "size": getattr(config, "size", None),
                        "distance": str(getattr(config, "distance", "Unknown")),
                    }
            elif vectors_param:
                vectors_info["default"] = {
                    "size": getattr(vectors_param, "size", None),
                    "distance": str(getattr(vectors_param, "distance", "Unknown")),
                }

        # ✅ Формируем ответ с явными ключами, которые ждёт фронтенд
        response = {
            "collection_name": collection_name,
            ...
            "vectors_config": vectors_info,
            "payload_fields": sorted(
                list(payload_fields)
            ),  # ✅ Сортировка для стабильности
            "field_types": field_types,
            "field_samples": field_samples,  # 🔍 Опционально: для отладки на фронтенде
            "status": str(getattr(info, "status", "unknown")),
        }

        # 🔍 Логирование для отладки
        app.logger.debug(
            f"Schema for {collection_name}: fields={response['payload_fields']}"
        )
        ...
        app.logger.error(
            f"Error in get_collection_schema({collection_name}): {e}", exc_info=True
        )
        return (
            jsonify(
                {
                    ...
                    "payload_fields": [],  # ✅ Возвращаем пустой массив, а не ошибку без данных
                }
            ),
            500,
        )
...
def get_sample_data(collection_name):
    ...
    try:
        ...
        samples = []
        for point in points:
            samples.append({"id": point.id, "payload": point.payload})
...
def generate_report():
    ...
    try:
        ...
        # Save report
        report_filename = (
            f"report_{collection_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
        ...
        if block.get("type") == "kpi":
            ...
            if "field" in block and block["field"]:
                ...
                if block.get("aggregation") == "sum":
                    ...
                    kpi["value"] = len(
                        set(str(v) for v in field_values if v is not None)
                    )
...
def generate_table_html(table_config, rows):
    ...
    for col in columns:
        header = (
            col.get("title", col.get("field", col)) if isinstance(col, dict) else col
        )
        html += f"                        <th>{header}</th>\n"
